...
(`~/.omp-cyberstrike/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.omp-cyberstrike/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.omp-cyberstrike/agent/sessions/ and populates ss_* tables
...
Verdicts are cached in `~/.omp-cyberstrike/stats-audit-cache.json` (keyed by session id +
