...
- **Custom adapter config**
  ...
  - Search order mirrors LSP config: project root, project config dirs (`.omp-cyberstrike/`, `.claude/`, `.codex/`, `.gemini/`), user config dirs (`~/.omp-cyberstrike/agent/`, `~/.claude/`, `~/.codex/`, `~/.gemini/`), plugin roots, then home-root fallback. Files are merged from lowest to highest priority.
...
Example `.omp-cyberstrike/dap.json`:
