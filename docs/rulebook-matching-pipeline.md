...
Loads `.omp-cyberstrike` rules from:

- project rules: `<cwd>/.omp-cyberstrike/rules/*.{md,mdc}` when the cwd's `.omp-cyberstrike/` directory is non-empty
- user rules: `<active-native-agent-dir>/rules/*.{md,mdc}`
- sticky user rule: `<active-native-agent-dir>/RULES.md`
- sticky project rule: `RULES.md` from the nearest non-empty `.omp-cyberstrike/` directory selected while walking from cwd toward the repository root; OMP does not continue farther when that directory lacks the file

The active native agent directory is `~/.omp-cyberstrike/agent` by default, follows named profiles, and honors `PI_CODING_AGENT_DIR`.
...
- `native` appends project `.omp-cyberstrike/rules`, user `~/.omp-cyberstrike/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
