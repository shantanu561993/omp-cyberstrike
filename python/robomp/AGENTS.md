...
- `docker-compose.yml` — `build.args.PI_BASE`, mounts `$PI_ROOT:/work/pi:ro`, `./data:/data`, `~/.omp-cyberstrike/agent/models.container.yml:ro` (mapped to `models.yml` inside the container — kept separate from the host's `~/.omp-cyberstrike/agent/models.yml` so the host omp doesn't pick up gateway routing intended only for the container), `extra_hosts: llm-gateway.internal:host-gateway`.
