...
import { autoLogin, handle2FA, loadSession, waitForManualLogin } from "./auth.ts";
...
import { planPage, planUnexploredElements, resolveModel } from "./navigator.ts";
...
// biome-ignore lint/correctness/noUnusedVariables: vendored from CyberStrike (AGPL) — retained for upstream parity
const SKIP_AUTO_DISCOVERY = /(logout|sign.?out|log.?out|delete.?account|reset.?data|revoke)/i;
	...
	while (taskQueue.length > 0 && steps < MAX_STEPS_PER_PAGE) {
		...
		// close the overlay first so the element becomes reachable again.
		{
			...
			if (!resolveElement(elements, lookupRole, lookupLabel) && (await isViewportCenterBlocked(page))) {
				log.debug("task target blocked by overlay, closing overlay first", {
					role: lookupRole,
					label: lookupLabel,
				});
		...
		log.info("executing task", {
			...
			label:
				task.type === "form"
					? `form(${task.fields.length} fields → ${task.submit.label.slice(0, 30)})`
					: task.label,
...
async function collectDOMLinks(page: Page, _pageUrl: string, inScope: ScopeMatcher): Promise<string[]> {
...
export async function run(config: AgentConfig): Promise<CrawlResult> {
	...
	if (config.auth.authenticated) {
		...
		await autoLogin(page, config.auth.credentials, { headless: config.headless });
	} else {
	...
	// OMP mechanical addition: export the authenticated session while the
	// context is still alive (session.json / cookie jar / auth headers).
	await config.auth.onCrawlEnd?.(context);
