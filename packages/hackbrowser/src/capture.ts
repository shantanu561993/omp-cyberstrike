...
import type { UIContext, UIField } from "./types.ts";
		...
		(triggerEl: Element | null): UIField[] => {
			...
			for (const el of inputs) {
				...
				results.push({
					...
					validation: {
						...
						maxLength:
							(el as HTMLInputElement).maxLength > 0 ? String((el as HTMLInputElement).maxLength) : undefined,
			...
			for (const sel of displaySelectors) {
				...
				for (const el of els) {
					...
					const name =
						el.getAttribute("data-field") || el.getAttribute("data-name") || el.getAttribute("id") || "";
