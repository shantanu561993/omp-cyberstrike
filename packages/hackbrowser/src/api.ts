// ATTRIBUTION: vendored from CyberStrike packages/hackbrowser (AGPL-3.0),
// source commit 71e14833cc2b003ed02837318e22bc769ddd8e21. Vendored
// unmodified except mechanical fixes required to run under bun/node (see
// ATTRIBUTION.md in the package root for the full list).
//
...
import { Log, type LogLevel, type LogSink } from "./log.ts";
import { clearEventSink, setEventSink } from "./panel/emit.ts";
import type { AgentConfig, CrawlResult, CredentialConfig, CSEvent } from "./types.ts";
...
export interface CrawlOptions {
	...
	// OMP mechanical addition: invoked with the live BrowserContext right
	// before teardown (session export — see ATTRIBUTION.md).
	onCrawlEnd?: (context: BrowserContext) => Promise<void>;
...
export type { LogLevel, LogRecord, LogSink } from "./log.ts";
