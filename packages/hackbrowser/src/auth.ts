...
export async function autoLogin(
  ...
  try {
    ...
    // OMP mechanical addition: in headless runs there is nobody to click the
    // manual-login button, so the fallback would block forever. Warn and
    // continue the crawl anonymous (deterministic) — seed the crawl at the
    // page containing the login form for auto-login to succeed.
    if (opts?.headless) {
