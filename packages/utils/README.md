...
| Module | Purpose |
| --- | --- |
| `logger` | Centralized logger writing to `~/.omp-cyberstrike/logs/` with rotation (TUI-safe — never stdout) |
| `prompt` | Handlebars-based prompt templating and formatting helpers |
| `dirs` | Path helpers for omp config directories (`~/.omp-cyberstrike`, XDG-aware on Linux) |
