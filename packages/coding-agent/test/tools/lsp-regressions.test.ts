...
/**
 * `loadConfig` walks the user config directories (~/.omp-cyberstrike/agent, ~/.pi/agent,
...
describe("lsp regressions", () => {
	...
	it("uses a custom server languageId for disk and in-memory document opens", async () => {
		...
		try {
			await Bun.write(
				path.join(tempDir.path(), ".omp-cyberstrike", "lsp.json"),
				JSON.stringify({
					servers: {
						"fake-gd": {
	...
	it("reload * invalidates the per-cwd config cache so newly written .omp-cyberstrike/lsp.json is observed", async () => {
		// #3546: `getConfig` caches the first `loadConfig` result per cwd
		// permanently. Creating `.omp-cyberstrike/lsp.json` after the first LSP call left
