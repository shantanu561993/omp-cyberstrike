...
//
// 1. `getConfigRootDir()` (`~/.omp-cyberstrike`)
...
//    or `../`-relative entry pointing somewhere else under `~/.omp-cyberstrike/`), plus
//    the legacy non-XDG `~/.omp-cyberstrike/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.omp-cyberstrike/agent` or `$PI_CODING_AGENT_DIR`)
...
//    Handles installed plugin extensions that live outside `~/.omp-cyberstrike` when
...
// symlinks `~/.omp-cyberstrike/agent/extensions/foo.ts` to a dotfiles repo would see the
