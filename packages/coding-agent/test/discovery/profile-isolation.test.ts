 ...
 * profile. A profile relocates the agent directory to ~/.omp-cyberstrike/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.omp-cyberstrike/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".omp-cyberstrike/agent") points at the tempdir decoys below; without the fix
