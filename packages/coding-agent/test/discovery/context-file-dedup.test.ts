...
describe("contextFileCapability.key", () => {
	...
	test("user-level files share the same key regardless of depth", () => {
		const a = makeContextFile({ path: "/home/user/.omp-cyberstrike/agent/AGENTS.md", level: "user" });
	...
	test("user key never collides with any project key", () => {
		const user = makeContextFile({ path: "/home/user/.omp-cyberstrike/AGENTS.md", level: "user" });
