...
test("project config.yml#extensions surfaces every sub-directory (#9768)", async () => {
	writeFile(path.join(project, ".omp-cyberstrike", "config.yml"), `extensions:\n  - "${ext}"\n`);
	await expectExtensionSubDirectoriesLoaded(ctx());
});
...
test("user config.yaml#extensions feeds sub-discovery", async () => {
	...
	writeFile(path.join(home, ".omp-cyberstrike", "agent", "config.yaml"), `extensions:\n  - "${ext}"\n`);
...
test("project config.yml#extensions replaces lower-precedence configured roots", async () => {
	...
	writeFile(path.join(home, ".omp-cyberstrike", "agent", "config.yml"), `extensions:\n  - "${userExt}"\n`);
	writeFile(
		path.join(project, ".omp-cyberstrike", "settings.json"),
		JSON.stringify({ extensions: [projectSettingsExt] }),
	);
	writeFile(path.join(project, ".omp-cyberstrike", "config.yml"), `extensions:\n  - "${projectConfigExt}"\n`);
...
test("user YAML config suppresses its legacy settings.json migration source", async () => {
	...
	writeFile(
		path.join(home, ".omp-cyberstrike", "agent", "settings.json"),
		JSON.stringify({ extensions: [legacyExt] }),
	);
	writeFile(path.join(home, ".omp-cyberstrike", "agent", "config.yml"), "theme:\n  dark: default\n");
...
test("scopeless reload with session roots equals the construction-time scoped load (#9769 invariant)", async () => {
	...
	const installed = path.join(home, ".omp-cyberstrike", "plugins", "node_modules", "invariant-installed");
