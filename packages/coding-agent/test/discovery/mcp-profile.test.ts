 ...
 *
 * A named profile relocates the agent directory to ~/.omp-cyberstrike/profiles/<name>/agent.
 * The native config provider used to read user-scope mcp.json from the literal
 * home (~/.omp-cyberstrike/agent/mcp.json) via `ctx.home`, so a profile never saw its own
 ...
 *
 * `os.homedir()` is mocked so the *old* code path (ctx.home + ".omp-cyberstrike/agent")
...
describe("native user-level MCP discovery follows the active profile", () => {
	...
	test("active profile loads its own user server, not the default profile's", async () => {
		// Active profile's agent dir (stand-in for ~/.omp-cyberstrike/profiles/<name>/agent).
		...
		await writeMcpJson(path.join(tempHome, ".omp-cyberstrike", "agent"), {
	...
	test("default profile loads the user server from ~/.omp-cyberstrike/agent", async () => {
		const defaultAgentDir = path.join(tempHome, ".omp-cyberstrike", "agent");
