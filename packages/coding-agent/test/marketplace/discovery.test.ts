 ...
 * depends on:
 *   1. OMP registry lives at path.join(home, ".omp-cyberstrike", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".omp-cyberstrike")
...
// but we know the value is ".omp-cyberstrike" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMP_CONFIG_DIR = ".omp-cyberstrike";
...
let tmpHome: string;
/** ~/.omp-cyberstrike/plugins/installed_plugins.json inside tmpHome */
...
describe("OMP registry path contract", () => {
	it("OMP registry lives at home/.omp-cyberstrike/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".omp-cyberstrike", "plugins", "installed_plugins.json");
