...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .omp-cyberstrike/ directory", async () => {
		// Layout: tmpDir/.omp-cyberstrike/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .omp-cyberstrike/ there.
		fs.mkdirSync(path.join(tmpDir, ".omp-cyberstrike"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".omp-cyberstrike", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .omp-cyberstrike/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.omp-cyberstrike/   +   tmpDir/sub/.omp-cyberstrike/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.omp-cyberstrike/, not climb further to tmpDir/.omp-cyberstrike/.
		fs.mkdirSync(path.join(tmpDir, ".omp-cyberstrike"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".omp-cyberstrike"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".omp-cyberstrike", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .omp-cyberstrike/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .omp-cyberstrike/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .omp-cyberstrike/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .omp-cyberstrike/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.omp-cyberstrike/.../installed_plugins.json.
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeOmpPath = path.join(homeDir, ".omp-cyberstrike", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		// Create .omp-cyberstrike/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".omp-cyberstrike", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".omp-cyberstrike", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".omp-cyberstrike", "plugins", "installed_plugins.json");
	});
