 ...
 *   bun scripts/bench-title-models.ts --db ~/.omp-cyberstrike/agent/history.db --out bench.json
 */
...
function parseArgs(argv: string[]): BenchConfig {
	...
	return {
		dbPath: (get("--db") ?? path.join(os.homedir(), ".omp-cyberstrike/agent/history.db")).replace(/^~/, os.homedir()),
