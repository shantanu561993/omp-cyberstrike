 ...
 *   2. `auth.broker.url` / `auth.broker.token` in `~/.omp-cyberstrike/agent/config.yml`
 *      (hidden from the settings UI; `!command` resolution supported).
 *   3. Token file `~/.omp-cyberstrike/auth-broker.token` (paired with URL from env or config).
 *
