 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.omp-cyberstrike/`.
 *    The first match returns `<dir>/.omp-cyberstrike/plugins/installed_plugins.json`.
 * 2. If no `.omp-cyberstrike/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.omp-cyberstrike/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .omp-cyberstrike/ directory (nearest wins).
	// Stop before os.homedir() — ~/.omp-cyberstrike/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.omp-cyberstrike/plugins/installed_plugins.json`
 * when no project anchor (.omp-cyberstrike/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .omp-cyberstrike/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * ~/.omp-cyberstrike/plugins/installed_plugins.json, plus the nearest project registry when present.
 *
...
export async function listClaudePluginRoots(
	...
	// Loaded from the nearest .omp-cyberstrike/plugins/installed_plugins.json relative to cwd.
