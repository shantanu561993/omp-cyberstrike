/**
 * Builtin Provider (.omp-cyberstrike)
 *
...
const DESCRIPTION = "Native OMP configuration from ~/.omp-cyberstrike and .omp-cyberstrike/";
const PRIORITY = 100;
...
async function getConfigDirs(ctx: LoadContext): Promise<Array<{ dir: string; level: "user" | "project" }>> {
	...
	// profile's agent dir (~/.omp-cyberstrike/profiles/<name>/agent), like sessions and MCP.
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .omp-cyberstrike/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.omp-cyberstrike/agent/skills/
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "Auto-generated managed skills from ~/.omp-cyberstrike/agent/managed-skills",
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// User scope:    ~/.omp-cyberstrike/agent/RULES.md
	// Project scope: nearest .omp-cyberstrike/RULES.md walking up from cwd to repoRoot
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .omp-cyberstrike/ directories",
	priority: PRIORITY,
