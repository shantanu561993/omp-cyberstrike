...
class AgentMetricRecorder {
	...
	constructor(meter: Meter) {
		...
		this.#chatCostUsd = meter.createCounter("pi.omp-cyberstrike.agent.chat.cost.estimated_usd", {
			description: "Estimated USD cost for completed chat calls.",
			unit: "USD",
		});
		this.#runs = meter.createCounter("pi.omp-cyberstrike.agent.runs", {
			description: "Completed agent runs.",
			unit: "{run}",
		});
		this.#steps = meter.createCounter("pi.omp-cyberstrike.agent.steps", {
			description: "Agent loop steps completed inside a run.",
			unit: "{step}",
		});
		this.#chatCalls = meter.createCounter("pi.omp-cyberstrike.agent.chat.calls", {
			description: "Chat calls completed inside agent runs.",
			unit: "{call}",
		});
		this.#chatDurationMs = meter.createHistogram("pi.omp-cyberstrike.agent.chat.duration", {
			description: "Total chat latency observed in an agent run.",
			unit: "ms",
		});
		this.#toolCalls = meter.createCounter("pi.omp-cyberstrike.agent.tool.calls", {
			description: "Tool calls completed inside agent runs.",
			unit: "{call}",
		});
		this.#toolDurationMs = meter.createHistogram("pi.omp-cyberstrike.agent.tool.duration", {
			description: "Total tool latency observed in an agent run.",
			unit: "ms",
		});
		this.#errors = meter.createCounter("pi.omp-cyberstrike.agent.errors", {
	...
	recordRun(summary: AgentRunSummary, coverage: AgentRunCoverage): void {
		const runAttrs = metricAttributes({
			"pi.omp-cyberstrike.agent.models_used.count": coverage.modelsUsed.length,
			"pi.omp-cyberstrike.agent.providers_used.count": coverage.providersUsed.length,
			"pi.omp-cyberstrike.agent.tools_available.count": coverage.toolsAvailable.length,
			"pi.omp-cyberstrike.agent.tools_invoked.count": coverage.toolsInvoked.length,
			"pi.omp-cyberstrike.agent.tools_unused.count": coverage.toolsUnused.length,
		});
		...
		for (const toolName in summary.tools.byName) {
			...
			for (const status of TOOL_STATUSES) {
				...
				if (count > 0)
					this.#toolCalls.add(count, metricAttributes({ ...toolAttrs, "pi.omp-cyberstrike.tool.status": status }));
			}
		}
...
function emitRunSummaryLog(summary: AgentRunSummary, coverage: AgentRunCoverage): void {
	emitOtelLog(
		...
		"agent run completed",
		{
			"pi.omp-cyberstrike.agent.step_count": summary.stepCount,
			"pi.omp-cyberstrike.agent.chats.total": summary.chats.total,
			"pi.omp-cyberstrike.agent.chats.total_latency_ms": summary.chats.totalLatencyMs,
			"pi.omp-cyberstrike.agent.tools.total": summary.tools.total,
			"pi.omp-cyberstrike.agent.tools.ok": summary.tools.ok,
			"pi.omp-cyberstrike.agent.tools.error": summary.tools.error,
			"pi.omp-cyberstrike.agent.tools.skipped": summary.tools.skipped,
			"pi.omp-cyberstrike.agent.tools.blocked": summary.tools.blocked,
			"pi.omp-cyberstrike.agent.tools.timeout": summary.tools.timeout,
			"pi.omp-cyberstrike.agent.tools.aborted": summary.tools.aborted,
			"pi.omp-cyberstrike.agent.tools.total_latency_ms": summary.tools.totalLatencyMs,
			"pi.omp-cyberstrike.agent.usage.input_tokens": summary.usage.inputTokens,
			"pi.omp-cyberstrike.agent.usage.output_tokens": summary.usage.outputTokens,
			"pi.omp-cyberstrike.agent.usage.cached_input_tokens": summary.usage.cachedInputTokens,
			"pi.omp-cyberstrike.agent.usage.cache_write_tokens": summary.usage.cacheWriteTokens,
			"pi.omp-cyberstrike.agent.usage.reasoning_output_tokens": summary.usage.reasoningOutputTokens,
			"pi.omp-cyberstrike.agent.usage.total_tokens": summary.usage.totalTokens,
			"pi.omp-cyberstrike.agent.cost.estimated_usd": summary.cost.estimatedUsd,
			"pi.omp-cyberstrike.agent.cost.unavailable_reasons": summary.cost.unavailableReasons.join(","),
			"pi.omp-cyberstrike.agent.errors.total": summary.errors.total,
			"pi.omp-cyberstrike.agent.coverage.tools_available": coverage.toolsAvailable.join(","),
			"pi.omp-cyberstrike.agent.coverage.tools_invoked": coverage.toolsInvoked.join(","),
			"pi.omp-cyberstrike.agent.coverage.tools_unused": coverage.toolsUnused.join(","),
			"pi.omp-cyberstrike.agent.coverage.models_used": coverage.modelsUsed.join(","),
			"pi.omp-cyberstrike.agent.coverage.providers_used": coverage.providersUsed.join(","),
		},
		"pi.omp-cyberstrike.agent.run.completed",
	);
}
...
function emitTelemetryWarningLog(warning: AgentTelemetryWarning): void {
	...
	emitOtelLog("warn", warning.message, attrs, "pi.omp-cyberstrike.telemetry.warning");
}
