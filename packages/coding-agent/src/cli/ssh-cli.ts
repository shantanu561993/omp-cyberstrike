...
async function handleList(cmd: SSHCommandArgs): Promise<void> {
	...
	if (hasProject) {
		process.stdout.write(chalk.bold("Project SSH Hosts (.omp-cyberstrike/ssh.json):\n"));
	...
	if (hasUser) {
		process.stdout.write(chalk.bold("User SSH Hosts (~/.omp-cyberstrike/agent/ssh.json):\n"));
