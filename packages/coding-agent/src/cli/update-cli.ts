...
async function fetchLatestManifest(
	...
	if (!response.ok) {
		if (response.status === 404 && channel === "canary") {
			throw new Error(`No canary release has been published for ${pkg} yet. Try \`${CLI_NAME} update --stable\`.`);
		}
...
export async function updateViaManager(
	...
	console.log(
		chalk.yellow(
			`\n${steps.manager} left no working ${CLI_NAME} ${release.version} launcher (${formatVerificationFailure(result, release.version)}); installing the standalone binary at ${launcherPath}.`,
		),
	);
	try {
...
${chalk.bold("Examples:")}
  ...
  ${CLI_NAME} update --canary    Switch to the canary channel and update
`);
}
