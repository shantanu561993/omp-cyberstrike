 ...
 *
 * Layout under `~/.omp-cyberstrike/wt/`:
 *
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.omp-cyberstrike/wt/`. */
	path: string;
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.omp-cyberstrike/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
