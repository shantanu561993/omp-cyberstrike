...
- Fixed native addon loading leaving stale `~/.omp-cyberstrike/natives/<version>` cache directories behind after updates; successful loads now remove older version directories best-effort.
...
- Updated native module loader to check `XDG_DATA_HOME` environment variable for native addon location before falling back to `~/.omp-cyberstrike/natives`
...
- Added automatic extraction of embedded native addon to `~/.omp-cyberstrike/natives/<version>` on first run for compiled binaries
...
- Embedded native addon payload for compiled binaries and extract to `~/.omp-cyberstrike/natives/<version>` on first run
